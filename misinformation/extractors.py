...
def extract_element(response, extract_spec):
    ...
    if method == 'xpath':
        ...
        if not elements:
            raise ValueError("Failed to extract element from {url} using xpath selector '{xpath}'".format(
...
def extract_datetime_string(date_string, format=None, timezone=False):
    try:
        ...
        try:
            ...
            if format:
                datetime = pendulum.from_format(date_string, format)
            else:
                # Assume ISO-8601
                datetime = pendulum.parse(date_string)
        except (ValueError, TypeError):
            # Otherwise try arrow as it can handle matching dates as substrings within a longer non-date string
            # TODO?: Drop arrow and translate format to regular expression to extract just date to pass to pendulum?
            if format:
                datetime = arrow.get(date_string, format)
            else:
                # Assume ISO-8601
                arrow.get(date_string)

        # Re-export as ISO-8601 string.
